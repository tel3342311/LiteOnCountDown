...
void CVolumeMeter::Render(ID2D1DeviceContext*d2ddc)
{
	...
	float fPeak = 0.f;
	if (m_pMICCapture != NULL)
		m_pMICCapture->GetPeakValue(&fPeak);
	GetGradientBrush(fPeak);
	if (m_pLinearGradientBrush != NULL)
		d2ddc->FillRectangle(D2D1::RectF(0, m_height * (1.f - fPeak), m_width, m_height), m_pLinearGradientBrush);
